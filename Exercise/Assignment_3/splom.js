var d3;
function drawSPLOM(svg, keys, data) {
  // adapted from https://observablehq.com/@d3/brushable-scatterplot-matrix
  // Specify the chart’s dimensions.
  const width = 528;
  const height = width;
  const padding = 28;
  const columns = keys.filter((k, i) => i < 3);
  const size =
  ...
  const cell = svg
    ...
    .attr(
      "transform",
      ([i, j]) => `translate(${(columns.length - i - 1) * size},${j * size})`
    );
  ...
  cell.call(brush, circles, svg, { padding, size, x, y, columns });
  ...
  document.addEventListener("selection", (e) => {
    circles.classed("hidden", (c) => !e.detail.selected["id-"+ c.id]);
  });
}
